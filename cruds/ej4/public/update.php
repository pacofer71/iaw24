...
mysqli_stmt_bind_result($stmt, $i, $nom, $des, $p, $ci);
...
if (isset($_POST['nombre'])) {
    ...
    //si estamos aqui todo correcto vamos a editar el articulo
    $q = "update articulos set nombre=?, descripcion=?, precio=?, categoria_id=? where id=?";
    $stmt = mysqli_stmt_init($llave);
    mysqli_stmt_prepare($stmt, $q);
    mysqli_stmt_bind_param($stmt, 'ssdii', $nombre, $descripcion, $precio, $categoria_id, $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    mysqli_close($llave);
    $_SESSION['mensaje'] = "Se editó el artículo";
    header("Location:articulos.php");
    exit;
}
?>
...
<body class="p-6 bg-purple-200">
    <h3 class="text-center font-bold text-xl">Editar Artículo</h3>
        ...
        <form action="update.php?id=<?= $id ?>" method="POST">
            ...
            <div>
                ...
                <textarea
                    ...
                    placeholder="Descripción del artículo"><?= $des ?></textarea>
                <?php
                ...
                <div class="relative">
                    <select
                        ...
                        <?php foreach ($categorias as $categoria):
                            $cadena = ($categoria['id'] == $ci) ? "selected" : "";
                        ?>
                            <option <?= $cadena ?> value="<?= $categoria['id']; ?>">
                                <?= $categoria['nombre']; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
